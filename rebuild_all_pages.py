  ...
  <div style="display:grid;grid-template-columns:1fr 420px;gap:52px;align-items:start;">
    ...
    <div>
      <div class="cal-card">
        ...
        <form id="bookingForm">
          <div class="bf-honey"><label>Leave blank<input type="text" name="website" tabindex="-1" autocomplete="off"></label></div>
          <!-- Hidden fields populated by calendar JS -->
          ...
          <div class="cal-time-wrap" id="calTimeWrap" style="display:none;">
            <p class="cal-time-label">Preferred Time — <span id="calSelectedDate"></span></p>
...
.cal-day {{ text-align:center; padding:9px 4px; font-size:13px; font-weight:600; color:#2b3340; cursor:pointer; border-radius:0; transition:background .15s,color .15s; position:relative; }}
...
(function(){{
  ...
  function render(){{
    ...
    for(var d=1;d<=days;d++){{
      ...
      if(selDate && dt.getTime()===selDate.getTime()) cls += ' cal-selected';
    ...
    grid.querySelectorAll('.cal-day:not(.cal-empty):not(.cal-past):not(.cal-sun)').forEach(function(el){{
      el.addEventListener('click', function(){{
        ...
        selTime = null;
        document.getElementById('calDateVal').value = '';
        document.getElementById('calTimeVal').value = '';
        render();
        var tw = document.getElementById('calTimeWrap');
        var sd = document.getElementById('calSelectedDate');
        tw.style.display = 'block';
        sd.textContent   = months[selDate.getMonth()] + ' ' + selDate.getDate() + ', ' + selDate.getFullYear();
        ...
        document.getElementById('calDateVal').value = sd.textContent;
      }});
    }});
  }}
  ...
  document.getElementById('calPrev').addEventListener('click', function(){{
    ...
    if(cur < new Date(today.getFullYear(), today.getMonth(), 1)) cur = new Date(today.getFullYear(), today.getMonth(), 1);
    render();
  }});
  ...
  document.querySelectorAll('.cal-slot').forEach(function(btn){{
    btn.addEventListener('click', function(){{
      ...
      document.getElementById('calTimeVal').value = selTime;
      document.getElementById('calFields').style.display = 'flex';
    }});
  }});

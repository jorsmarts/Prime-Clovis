...
/* ── Collect fields — handles all form layouts ──
 *   Layout A (homepage):       full_name, zip_code, appliance_type, issue_description
 *   Layout B (inner pages):    name, zip, appliance, issue
 *   Layout C (booking/contact): fname, lname, phone, email, appliance, issue
 */
$fname     = clean($_POST['fname']     ?? $data['fname']     ?? '');
$lname     = clean($_POST['lname']     ?? $data['lname']     ?? '');

// Accept "name" (inner-page mini-form) or "full_name" (homepage) as a fallback
$full_name = clean($_POST['full_name'] ?? $data['full_name']
...
// "zip_code" (homepage) → same as "zip" (other forms)
$zip       = clean($_POST['zip']            ?? $data['zip']
          ?? $_POST['zip_code']             ?? $data['zip_code']        ?? '');

// "appliance_type" (homepage) → same as "appliance" (other forms)
$appliance = clean($_POST['appliance']      ?? $data['appliance']
          ?? $_POST['appliance_type']       ?? $data['appliance_type']  ?? '');

$brand     = clean($_POST['brand']          ?? $data['brand']          ?? '');
$date      = clean($_POST['date']           ?? $data['date']           ?? '');
$time      = clean($_POST['time']           ?? $data['time']           ?? '');

// "issue_description" (homepage) → same as "issue" (other forms)
$issue     = clean($_POST['issue']          ?? $data['issue']
          ?? $_POST['issue_description']    ?? $data['issue_description'] ?? '');
